                ...
                if (quantity > 0) {
                    ...
                    if (cart != null) {
                        ...
                        if (dao.checkQuantity(id, quantity) == false) {
                            String error = "This product " + id + " is out of stock or over available quantity";
                            request.setAttribute("ERROR_CART", error);
                        }
                        for (ProductDTO product : cart.getCart().values()) {
                            if (product.getID().equals(id)) {
                                dto = new ProductDTO(id, product.getName(), quantity, product.getPrice(), product.getDescription(), product.getCategory(), product.getImage(), true);
                                break;
                            }
                        }
                        cart.update(dto);
